...
TASKS
1. Read the Debugger's investigation report in `ai-chat/investigations/INV-...md`.
...
2. Testing & Self-Verification (MANDATORY)
   ...
   - **Coder Circuit Breaker**: If you fail to pass the Self-Verification Plan after 3 attempts, you must STOP modifying code. Revert your breaking changes, document the exact failure in the `ai-chat/investigations/INV-...md` file, and output `HUMAN_INTERVENTION_REQUIRED: Escalating back to Debugger` to halt execution.
